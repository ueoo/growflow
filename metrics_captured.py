...
def img2vid(path, img_files, is_reverse=False, video_duration=3):
    ...
    ffmpeg_cmd = [
        ...
        "-y",
        "-framerate",
        f"{num_images/video_duration}",  # Set framerate (adjust as needed)
        "-i",
...
def evaluate(cfg, data_dir, method_paths, output_path, split="test", use_mask_psnr=True):
    ...
    fixed_bkgd = torch.tensor(cfg.bkgd_color, device="cuda")[None, :] / 255.0  # NOTE: always use black
    ...
    shared_dataset = Dynamic_Datasetshared(
        parser=parser, apply_mask=cfg.apply_mask, debug_data_loading=cfg.debug_data_loading
    )
    ...
    for test_folder in method_paths:
        ...
        for camera_folder in camera_folders:
            ...
            for i, img_file in enumerate(img_files):
                ...
                if use_mask_psnr:
                    gt_mask = (
                        torch.from_numpy(testset.timestep_masks[timestep][actual_camera_index])
                        .to(rendered_img)
                        .to(torch.bool)
                    )
                else:
                ...
                results[method_name]["psnr"][camera_folder][timestep] = round(
                    psnr(rendered_img, eval_pixels, gt_mask).item(), 2
                )
                if timestep in training_times_int:
                    results_training[method_name]["psnr"][camera_folder][timestep] = round(
                        psnr(rendered_img, eval_pixels, gt_mask).item(), 2
                    )
                elif timestep in interp_times_int:
                    results_interp[method_name]["psnr"][camera_folder][timestep] = round(
                        psnr(rendered_img, eval_pixels, gt_mask).item(), 2
                    )
                ...
                results[method_name]["ssim"][camera_folder][timestep] = round(
                    ssim(rendered_img_permuted, eval_pixels_permuted).item(), 3
                )
                results[method_name]["lpips"][camera_folder][timestep] = round(
                    lpips(rendered_img_permuted, eval_pixels_permuted).item(), 3
                )
                if timestep in training_times_int:
                    results_training[method_name]["ssim"][camera_folder][timestep] = round(
                        ssim(rendered_img_permuted, eval_pixels_permuted).item(), 3
                    )
                    results_training[method_name]["lpips"][camera_folder][timestep] = round(
                        lpips(rendered_img_permuted, eval_pixels_permuted).item(), 3
                    )
                elif timestep in interp_times_int:
                    results_interp[method_name]["ssim"][camera_folder][timestep] = round(
                        ssim(rendered_img_permuted, eval_pixels_permuted).item(), 3
                    )
                    results_interp[method_name]["lpips"][camera_folder][timestep] = round(
                        lpips(rendered_img_permuted, eval_pixels_permuted).item(), 3
                    )
...
if __name__ == "__main__":
    ...
    for scene in ["pi_paperwhite_full_subset4"]:
        ...
        if not cfg.skip_dynamic3dgs:
            method_paths.append(f"../baselines/output_arxiv/Dynamic3DGS/{scene}/test")
        if not cfg.skip_4dgs:
            method_paths.append(f"../baselines/output_arxiv/4dgs/{scene}/test")
        if not cfg.skip_4dgaussians:
            method_paths.append(f"../baselines/output_arxiv/4dgaussians/{scene}/test")
        ...
        if cfg.existing_result_path != "":
            ...
            results, results_training, results_interp = evaluate(
                cfg, data_dir, method_paths, output_path=output_path, split="test", use_mask_psnr=cfg.use_mask_psnr
            )
        ...
        if not cfg.skip_rendering:
            ...
            all_camera_names = [
                f for f in os.listdir(method_paths[-1]) if f.startswith("r_")
            ]  # just list one of the method paths for camera names
            for camera in all_camera_names:
                ...
                ffmpeg_cmd.extend(
                    [
                        ...
                        # "-vsync", "2",
                        "-r",
                        "15",  # Force a consistent output framerate
                        "-c:v",
                        "libx264",
                        "-pix_fmt",
                        "yuv420p",
