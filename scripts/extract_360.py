...
def process_timelapse_folders(base_dir=".", fps=30, max_workers=4, skip_existing=True):
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        # Submit all tasks
        future_to_folder = {
            executor.submit(process_single_folder, folder, fps, skip_existing): folder for folder in timelapse_folders
        }
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dir", default=".", help="Base directory to search for timelapse folders (default: current directory)"
    )
    parser.add_argument("--fps", type=int, default=21, help="Frames per second for output video (default: 30)")
    parser.add_argument("--workers", type=int, default=4, help="Number of worker threads (default: 4)")
    parser.add_argument(
        "--no-skip", action="store_true", help="Do not skip folders with existing videos (recreate all)"
    )

    args = parser.parse_args()

    process_timelapse_folders(args.dir, args.fps, args.workers, skip_existing=not args.no_skip)
