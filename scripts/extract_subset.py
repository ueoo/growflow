...
def extract_subset_json(base_path, target_path, start, end):
    ...
    for i in range(start, end + 1):
        ...
        shutil.copy(
            mesh_file, os.path.join(target_path, "meshes", "relevant_meshes", f"mesh{cached_mesh_list[i]:04d}.obj")
        )
    ...
    new_json_train = {"camera_angle_x": train_json["camera_angle_x"], "frames": []}
    ...
    new_json_test = {"camera_angle_x": test_json["camera_angle_x"], "frames": []}
...
if __name__ == "__main__":
    ...
    print(f"extracting subset renderings for {scene} for {start} - {end}")
    base_path = (
        f"/scratch/ondemand28/weihanluo/neural_ode_splatting/data/dynamic/blender/360/multi-view/30_views/{scene}"
    )
    target_path = f"/scratch/ondemand28/weihanluo/neural_ode_splatting/data/dynamic/blender/360/multi-view/30_views/{scene}_subset_{start}_{end}"
    os.makedirs(target_path, exist_ok=True)
    extract_subset_json(base_path, target_path, start, end)
