...
def run_dense_point_cloud(base_dir, scene, date, downsample_factor=1):
    ...
    undistort_cmd = (
        f"colmap image_undistorter --image_path {images_path} --input_path {sparse_path} --output_path {dense_path}"
    )
    ...
    fusion_cmd = (
        f"colmap stereo_fusion --workspace_path {dense_path} --output_path {os.path.join(dense_path, 'fused.ply')}"
    )
...
def split_colmap_results(base_dir, scene, dates, image_to_date_mapping, sparse_output_folder):
    ...
    export_cmd = (
        f"colmap model_converter --input_path {main_recon_path} --output_path {temp_txt_folder} --output_type TXT"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-c", default=True, help="Whether to create the combined image, set to False if u dont want to rewrite"
    )
    ...
    scene = args.scene
    dates = sorted(
        [f for f in os.listdir(os.path.join(base_dir, scene)) if f.startswith("timelapse")]
    )  # make sure to name the folders timelapse*
    ...
    try:
        ...
        if args.overwrite:
            ...
            sparse_folder = run_colmap_multi_date(
                base_dir, scene, dates, downsample_factor, use_sequential, create_combined, use_mask, fixing_intrinsics
            )
