...
from models.encoding import exp_se3, get_embedder
...
class Simple_ODE(nn.Module):
    ...
    def __init__(
        ...
        self.output_dim = (
            feature_dim + augment_dim
        )  # The output should include the augmented dimension, since we are also augmenting the y0
        self.hidden_dim = hidden_dim
        self.hidden_depth = hidden_depth
        self.non_linearity_name = non_linearity_name
        self.encoding_name = encoding_name
        self.nfe = 0  # Number of function evaluations
        self.concat_remaining = (
            concat_remaining  # whether to concat the remaining parameters when inputting to the neural ode
        )
        ...
        self.absolute_times = (
            None  # absolute times denote the absolute times of the initial conditions (and of their integration steps)
        )
        ...
        if self.encoding_name == "freq":
            if self.use_timenet:
                ...
                self.timenet = nn.Sequential(
                    nn.Linear(time_input_ch, self.hidden_dim),
