...
from lib.torchdiffeq import odeint, odeint_adjoint
...
class ODEBlock(nn.Module):
    ...
    def forward(self, x, eval_times=None):
        ...
        if self.adjoint:
            out = odeint_adjoint(
                self.odefunc, x_aug, integration_time, rtol=self.rtol, atol=self.atol, method=self.method
            )
        else:
            out = odeint(
                self.odefunc, x_aug, integration_time, rtol=self.rtol, atol=self.atol, method=self.method
            )  # (T, N, n_enc_features-3+feature_dim+augment_dim)
...
class DynamicalModel(nn.Module):
    ...
    def __init__(
        ...
        assert self.encoding in [
            ...
            "hexplane",
            "",
        ], f"encoding must be one of freq, ingp or None, it is set to {self.encoding}"
        ...
        if self.encoding == "hexplane":
            ...
            encoder_output_dim = (
                xyz_input_ch + time_input_ch
            )  # the reason we subtract 3 here is because we need to account for removing gaussian means
