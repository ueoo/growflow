...
from helpers.pc_viz_utils import animate_point_clouds_lst, visualize_point_cloud
...
def img2vid(path, img_files, is_reverse=False):
    ...
    ffmpeg_cmd = [
        ...
        "-y",
        "-framerate",
        f"{num_images/cfg.video_duration}",  # Set framerate (adjust as needed)
        "-i",
    ...
    ffmpeg_cmd.extend(
        [
            ...
            "-crf",
            "23",  # Quality (lower is better, 18-28 is good range)
            "-vsync",
            "2",
            output_video_path,
        ]
...
def img2vid_gt(path, img_files, is_reverse=False, bg_color=(255, 255, 255)):
    ...
    ffmpeg_cmd = [
        ...
        "-y",
        "-framerate",
        f"{num_images/cfg.video_duration}",
        "-i",
        temp_input_pattern,
    ]
    ...
    ffmpeg_cmd.extend(["-c:v", "libx264", "-pix_fmt", "yuv420p", "-crf", "23", "-vsync", "2", output_video_path])
            ...
            if metric != "chamfer":
                ...
                for timestep, value in results[method][metric].items():
                    if timestep == "average_chamfer":
                        continue
                    integer_time = int(timestep)
                    detailed_data.append({"Method": method, "Metric": "CD", "Timestep": integer_time, "Value": value})
            ...
            if metric != "chamfer":
                    ...
                    if isinstance(results[method][metric][camera], dict):
                        for timestep, value in results[method][metric][camera].items():
                            try:
                                integer_time = int(timestep)
                                method_data.append(
                                    {"Metric": metric.upper(), "Camera": camera, "Timestep": timestep, "Value": value}
                                )
                            except ValueError:
                                continue
                        ...
                        method_df.to_csv(
                            os.path.join(output_path, method, f"{method}_{camera}_{metric}_results.csv"), index=False
                        )
                ...
                if isinstance(results[method][metric], dict):
                    for timestep, value in results[method][metric].items():
                        try:
                            integer_time = int(timestep)
                            chamfer_data.append({"Metric": metric.upper(), "Timestep": timestep, "Value": value})
                        except ValueError:
                            continue
                        values.append(value)
...
def evaluate(cfg, data_dir, method_paths, output_path, split="test"):
    ...
    psnr = psnr_metric
    lpips = LearnedPerceptualImagePatchSimilarity(net_type="vgg", normalize=True).to(
        device
    ...
    for test_folder in method_paths:
        assert test_folder.split("/")[-1] in [
            "test",
            "test_masked",
            "test_white",
        ], "please ensure the input file is test/ "
    ...
    for test_folder in method_paths:
        ...
        results[method_name] = {"psnr": {}, "ssim": {}, "lpips": {}, "chamfer": {}}  # Added Chamfer distance
        results_interp[method_name] = {"psnr": {}, "ssim": {}, "lpips": {}, "chamfer": {}}  # Added Chamfer distance
        results_training[method_name] = {"psnr": {}, "ssim": {}, "lpips": {}, "chamfer": {}}  # Added Chamfer distance
        ...
        if point_cloud_trajectory_filename in os.listdir(test_folder):
            ...
            point_cloud_trajectory = torch.load(
                os.path.join(test_folder, point_cloud_trajectory_filename), weights_only=False
            )
            ...
            assert len(mesh_indices) == len(
                point_cloud_trajectory
            ), "make sure the number of mesh indices equals the number of timesteps"
            ...
            gt_tracks_path = os.path.join(
                data_dir, "meshes", f"relevant_{scene}_meshes", "trajectory_frames.npz"
            )  # using trajectory_frames is correct, cause those indicies are used for tracking.
            ...
            for t, mesh_index in enumerate(mesh_indices):  # mesh_index is reversed here
                ...
                gt_mesh_path = os.path.join(
                    data_dir, "meshes", f"relevant_{scene}_meshes", f"mesh_{converted_integer}.ply"
                )
                chamfer_distance, _ = compute_chamfer_between_point_and_mesh(
                    point_cloud_trajectory,
                    gt_mesh_path,
                    t,
            ...
            results[method_name]["chamfer"]["average_chamfer"] = round(
                sum(chamfer_distance_lst) / len(chamfer_distance_lst), 3
            )
            ...
            print(
                f"method {method_name} has obtained an average chamfer of {round(sum(chamfer_distance_lst)/len(chamfer_distance_lst), 3)}"
            )
        ...
        for metric in ["psnr", "ssim", "lpips"]:  # Skip chamfer since average is already computed before
            ...
            for camera_folder in results[method_name][metric]:  # loops over each camera
                ...
                if camera_values:
                    ...
                    results[method_name][metric][camera_folder]["average_time_" + metric] = round(
                        sum(camera_values) / len(camera_values), NUM_SIG
                    )
                    all_cameras_avg.append(round(sum(camera_values) / len(camera_values), NUM_SIG))

                    results_interp[method_name][metric][camera_folder]["average_time_" + metric] = round(
                        sum(interp_time_cam_values) / len(interp_time_cam_values), NUM_SIG
                    )
                    interp_time_cameras_avg.append(
                        round(sum(interp_time_cam_values) / len(interp_time_cam_values), NUM_SIG)
                    )
                    results_training[method_name][metric][camera_folder]["average_time_" + metric] = round(
                        sum(train_time_cam_values) / len(train_time_cam_values), NUM_SIG
                    )
                    train_time_cameras_avg.append(
                        round(sum(train_time_cam_values) / len(train_time_cam_values), NUM_SIG)
                    )

            # Calculate average across all cameras
            if all_cameras_avg:
                results[method_name][metric]["average_cam_" + metric] = round(
                    sum(all_cameras_avg) / len(all_cameras_avg), NUM_SIG
                )
                results_training[method_name][metric]["average_cam_" + metric] = round(
                    sum(train_time_cameras_avg) / len(train_time_cameras_avg), NUM_SIG
                )
                results_interp[method_name][metric]["average_cam_" + metric] = round(
                    sum(interp_time_cameras_avg) / len(interp_time_cameras_avg), NUM_SIG
                )
...
def process_scene(scene):
    ...
    if not cfg.skip_dynamic3dgs:
        if cfg.bkgd_color == [1, 1, 1]:
            method_paths.append(f"../baselines/output/Dynamic3DGS/{scene}/test_white")
        else:
            method_paths.append(f"../baselines/output_cvpr/Dynamic3DGS/{scene}/test")

    if not cfg.skip_4dgs:
        if cfg.bkgd_color == [1, 1, 1]:
            method_paths.append(f"../baselines/output/4dgs/{scene}/test_white")
        else:
            method_paths.append(f"../baselines/output_cvpr/4dgs/{scene}/test")

    if not cfg.skip_4dgaussians:
        if cfg.bkgd_color == [1, 1, 1]:
            method_paths.append(f"../baselines/output/4dgaussians/{scene}/test_white")
        else:
            method_paths.append(f"../baselines/output_cvpr/4dgs/{scene}/test")
    ...
    if cfg.existing_result_path != "":
        ...
        results, results_training, results_interp = evaluate(
            cfg, data_dir, method_paths, output_path=output_path, split="test"
        )
    ...
    if not cfg.skip_rendering:
        ...
        for camera in all_camera_names:
            # generating gt video
            img2vid_gt(
                f"{data_dir}/test/{camera}", os.listdir(f"{data_dir}/test/{camera}"), bg_color=bg_color
            )  # Always run img2vid, since we use newer arg vsync
            ...
            ffmpeg_cmd.extend(
                [
                    ...
                    "main",
                    "-level",
                    "4.0",  # Constrains the encoding parameters
    ...
    if "clematis" in scene:
        ...
        center_position = [6.3185021e-04, 5.7011396e-03, 1.6463835e00]
...
if __name__ == "__main__":
    ...
    assert cfg.bkgd_color == [1, 1, 1] or cfg.bkgd_color == [
        0,
        0,
        0,
    ], "support metrics code only on white/black background"
    ...
    scenes = [
        "clematis_transparent_final_small_vase_70_timesteps_subsample_6",
