...
def _resolve_camera_angle_x(transforms: Dict[str, Any]) -> float:
    """
    Support both blender json styles:
    - top-level camera_angle_x
    - per-frame camera_angle_x
    """
    if "camera_angle_x" in transforms:
        return float(transforms["camera_angle_x"])
    frames = transforms.get("frames", [])
    if len(frames) > 0 and "camera_angle_x" in frames[0]:
        return float(frames[0]["camera_angle_x"])
    raise KeyError("camera_angle_x is missing from both top-level and frame entries in transforms json")
...
class Static_Dataset:
    ...
    def __post_init__(self):
        ...
        cx = image_width / 2.0
        cy = image_height / 2.0
        camera_angle_x = _resolve_camera_angle_x(transforms)
        fl = 0.5 * image_width / np.tan(0.5 * camera_angle_x)
        self.intrinsics = torch.tensor([[fl, 0, cx], [0, fl, cy], [0, 0, 1]], dtype=torch.float32)
...
class Dynamic_Dataset(Dataset):
    ...
    def __init__(
        ...
        all_times = []
        for frame in transforms["frames"]:
            camera_id = f"r_{int(frame['view_idx'])}" if "view_idx" in frame else frame["file_path"].split("/")[-2]
        ...
        for frame in transforms["frames"]:
            ...
            camera_id = f"r_{int(frame['view_idx'])}" if "view_idx" in frame else image_ids.split("/")[-2]
        ...
        all_meshes = sorted(os.listdir(mesh_path)) if mesh_path.exists() else []
        ...
        self.first_mesh_path = str(mesh_path / all_meshes[-1]) if len(all_meshes) > 0 else ""
        ...
        # see also
        # https://github.com/nerfstudio-project/nerfstudio/blob/main/nerfstudio/data/dataparsers/blender_dataparser.py
        image_height, image_width = self.images[list(self.images.keys())[0]][0].shape[
            :2
        ]  # select the first image from first view
        cx = image_width / 2.0
        cy = image_height / 2.0
        camera_angle_x = _resolve_camera_angle_x(transforms)
        fl = 0.5 * image_width / np.tan(0.5 * camera_angle_x)
    ...
    def __getitem__(self, timestep: int) -> Dict[str, Any]:
        ...
        """
        data = {"K": self.intrinsics, "camtoworld": {}, "image": {}, "image_id": {}}
        if self.return_mask:
            data["mask"] = {}
