...
from helpers.utils import (
    safe_state,
    setup_directories,
    verify_optimizer_parameter_references,
)
    ...
    def __init__(self, cfg) -> None:
        ...
        if cfg.data_type == "colmap":
            ...
            self.shared_dataset = Dynamic_Datasetshared(
                parser=self.parser, apply_mask=cfg.apply_mask, debug_data_loading=cfg.debug_data_loading
            )
    ...
    def run(self):
        ...
        if cfg.dynamic_ckpt is None:
            ...
            if cfg.static_ckpt is None:
                ...
                ckpts = [torch.load(file, map_location=self.device, weights_only=False) for file in cfg.static_ckpt]
            ...
            full_trajectory = (
                np.load(cfg.full_trajectory_path, allow_pickle=True) if cfg.full_trajectory_path != "" else None
            )
            ...
            ckpts = [torch.load(file, map_location=self.device, weights_only=False) for file in cfg.dynamic_ckpt]
            ...
            if cfg.resume_dyn_training:
                ...
                if cfg.load_optimizers:
                    ...
                    if "encoder_optimizer_state_dict" in ckpts[0].keys():
                        if cfg.encoding in ["hexplane", "ingp"]:
                            self.trainer.dynamical_model.optimizers["encoder_optimizer"].load_state_dict(
                                ckpts[0]["encoder_optimizer_state_dict"]
                            )
                            self.trainer.dynamical_model.schedulers["encoder_scheduler"].load_state_dict(
                                ckpts[0]["encoder_scheduler_state_dict"]
                            )

                    self.trainer.dynamical_model.optimizers["neural_ode_optimizer"].load_state_dict(
                        ckpts[0]["neural_ode_optimizer_state_dict"]
                    )
                    self.trainer.dynamical_model.schedulers["neural_ode_scheduler"].load_state_dict(
                        ckpts[0]["neural_ode_scheduler_state_dict"]
                    )
                ...
                if cfg.version == "ours3":
                    ...
                    self.trainer.train_split(
                        previous_init_params=previous_init_params, init_step=step, timestep_counter=timestep_counter
                    )
                else:
