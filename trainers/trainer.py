...
from datasets.sampler import (
    InfiniteNeuralODEDataSampler,
    NeuralODEDataSampler,
    NeuralODEDataSampler_MixedInit,
)
...
class Trainer(BaseEngine):
    ...
    def train_static(self, time_index):
        ...
        if cfg.data_type == "blender":
            ...
            self.single_timetrainset = SingleTimeDataset(
                self.trainset, time_index
            )  # wrap the single time dataset by itself
        else:
        ...
        for step in pbar:
            ...
            if (
                time_index == 0
            ):  # reason is if we're training per timestep gaussians, we want to use same number of gaussians, only prune when t=0
            ...
            if cfg.opacity_reg > 0.0:
                loss = loss + cfg.opacity_reg * torch.abs(torch.sigmoid(self.gaussians.splats["opacities"])).mean()
            if cfg.scale_reg > 0.0:
                loss = loss + cfg.scale_reg * torch.abs(torch.exp(self.gaussians.splats["scales"])).mean()
            ...
            if step in [i - 1 for i in cfg.static_save_steps] or step == max_steps - 1:
                ...
                torch.save(data, f"{self.ckpt_dir}/gaussian_ckpt_{step}_t{time_index}.pt")

            if step in [i - 1 for i in cfg.static_ply_steps] or step == max_steps - 1 and cfg.static_save_ply:
                rgb = None
            ...
            if cfg.visible_adam:
                ...
                if cfg.packed:
                    visibility_mask = torch.zeros_like(self.gaussians.splats["opacities"], dtype=bool)
    ...
    def train_dynamic_captured(
        self,
        init_step=0,
        timestep_counter=None,
        timestep_renders=None,
        lst_of_prog_indices=None,
        next_t=None,
        ...
        while temp_batch_size >= self.trainset.num_timesteps():
            temp_batch_size -= 1
        print(
            f"we are training with a temporal batch size of {temp_batch_size} and a camera batch size of {cfg.cam_batch_size}"
        )

        if full_trajectory is None:
            custom_sampler = NeuralODEDataSampler(
                self.trainset, train_time_index, shuffle=cfg.shuffle_ind
            ...
            prog_sampler = NeuralODEDataSampler_MixedInit(
                self.trainset,
                train_time_index,
                seq_length=cfg.temp_batch_size,
        ...
        if cfg.use_bounding_box:
            ...
            cfg.use_mask_proj
        ):  # in this case, load all the masks, apply dilation and project the 3DGaussians onto the viewpoint of each of the masks
            print("using mask projection segmentation")
            if (
                cfg.learn_masks
            ):  # in this case, we have trained a set of gaussian masks, so we can directly use them for 3D segmentations
                ...
                t0 = 0
                Ks = torch.from_numpy(self.trainset.timestep_intrinsics[t0][1])[None].to(
                ...
                for pose_idx in self.trainset.timestep_poses[t0].keys():
                    ...
                    if cfg.use_own_impl:
                        ...
                        _, _, infos = self.gaussians.rasterize_quick_captured(
                            c2w=c2ws[None], K=Ks, width=width, height=height
                        )
                    ...
                    gaussians_in_mask = torch.where(valid_mask)[0][
                        mask_values > 0
                    ]  # Assuming mask > 0 indicates foreground
                ...
                if use_intersection:
                    ...
                    if per_viewpoint_gaussian_sets:
                        ...
                        final_gaussian_indices = torch.tensor(
                            list(intersection_gaussian_indices), device="cuda", dtype=torch.long
                        )
                        print(
                            f"Intersection: Found {len(final_gaussian_indices)} gaussians present in all {len(per_viewpoint_gaussian_sets)} viewpoints"
                        )
                    else:
                    ...
                    final_gaussian_indices = torch.tensor(
                        list(union_gaussian_indices), device="cuda", dtype=torch.long
                    )
        ...
        for step in pbar:
            if self.dynamical_model.__class__.__name__ not in [
                "deform_network",
                "DeformModel",
            ]:  # only show nfe for odes
            ...
            if cfg.mixed_init_training:
                if step % cfg.global_integration_interval == 0 and step >= cfg.global_integration_start and train_all:
                    ...
                    if cfg.num_init_conditions == 1:
                        ...
                        previous_params = torch.from_numpy(full_trajectory[previous_index]).to(
                            "cuda"
                        )  # initial_params
                        inp_t[0] = previous_t
                        selected_gaussians = previous_params[bounding_box_mask]  # select a subset of gaussians
                        # print(f"inp_t is {inp_t}")
                        pred_param_selected = self.dynamical_model(
                            selected_gaussians, inp_t
                        )  # (T, N_gaussians, feat_dim)
                        T = pred_param_selected.shape[0]
                        pred_param = fixed_initial_params.unsqueeze(0).repeat(T, 1, 1)
                        pred_param[:, bounding_box_mask] = pred_param_selected
                        # end_time = time.time()
            ...
            if cache_trajectory:
                full_trajectory[current_indices] = (
                    pred_param_novel.detach().cpu().numpy()
                )  # updates all timesteps in the batch

            colors, alphas = self.gaussians.rasterize_with_dynamic_params_batched(
                pred_param_novel, raster_params, activate_params=True
            )
            ...
            if cfg.compute_tv_loss_ours:
                loss += self.dynamical_model.encoding_network_pos.compute_regularization(
                    cfg.time_smoothness_weight_ours, cfg.l1_time_planes_weight_ours, cfg.plane_tv_weight_ours
                )

            if cfg.param_loss_reg > 0:
                param_loss = cfg.param_loss_reg * torch.nn.functional.l1_loss(
                    pred_param_novel.squeeze(), full_trajectory_torch[first_predicted_index]
                )
                loss += param_loss
            ...
            if cfg.debug:
                if step % cfg.debug_every == 0:
                    ...
                    try:
                        ...
                        print(
                            f"average psnr on test camera 0 is {sum(psnr_ours_dict['test'])/len(psnr_ours_dict['test'])}"
                        )
            ...
            if cfg.wandb_every > 0 and step % cfg.wandb_every == 0:
                ...
                self.wandb_run.log({"train_dyn/loss": loss.item(), "train_dyn/mem": mem}, step=step)
            ...
            if step in [i - 1 for i in cfg.dynamic_save_steps] or step == max_steps - 1:
                ...
                if self.dynamical_model.encoding in ["hexplane", "ingp"]:
                    data = {
                        ...
                        "neural_ode_optimizer_state_dict": self.dynamical_model.optimizers[
                            "neural_ode_optimizer"
                        ].state_dict(),
                        "neural_ode_scheduler_state_dict": self.dynamical_model.schedulers[
                            "neural_ode_scheduler"
                        ].state_dict(),
                        "encoder_optimizer_state_dict": self.dynamical_model.optimizers[
                            "encoder_optimizer"
                        ].state_dict(),
                        "encoder_scheduler_state_dict": self.dynamical_model.schedulers[
                            "encoder_scheduler"
                        ].state_dict(),
                ...
                torch.save(data, f"{self.ckpt_dir}/neural_ode_{step}.pt")

                # #run eval
            if step in [i - 1 for i in cfg.dynamic_eval_steps]:
                try:
                    self.evaluator.dynamic_eval_captured(
                        fixed_initial_params, step, debug_raster_params, bounding_box_mask
                    )
                except Exception as e:
    ...
    def generate_trajectory(self):
        ...
        if cache_trajectory:
            ...
            full_trajectory = np.zeros(
                (len(cont_times), fixed_initial_params.shape[0], fixed_initial_params.shape[1]), dtype=np.float32
            )
        ...
        cfg.progressive_training_intervals = list(
            range(
                cfg.min_iterations_req, cfg.min_iterations_req * self.trainset.num_timesteps(), cfg.min_iterations_req
            )
        )  # NOTE: just using this for quick testing
        ...
        num_train_all = (
            cfg.num_train_all
        )  # NOTE: this is the number of iterations we want to train on all timesteps, maybe make this longer
        ...
        desired_num_steps = {
            i: per_time_iter for i in range(1, len(train_time_index) + 1)
        }  # uniformly distribute max_steps amongst num_timesteps
        ...
        prog_sampler = InfiniteNeuralODEDataSampler(self.trainset, lst_of_prog_indices, shuffle=cfg.shuffle_ind)
        ...
        if cfg.data_type == "colmap":
            ...
            assert (
                sum([cfg.use_bounding_box, cfg.use_mask_proj]) <= 1
            ), "Either use bounding box or mask proj or neither"
            if cfg.use_bounding_box:
                number_of_times = len(self.trainset.parser.timestep_data)
                if "pi_corn_full_subset4" in cfg.data_dir:
                    box_center = [0.093149, 0.148414, -0.293219]
        ...
        for step in pbar:
            ...
            for ts_idx in active_timesteps.copy():
                ...
                if timestep_counter[ts_idx] == desired_num_steps[ts_idx]:  # sometimes u might have allocated more
                    ...
                    if len(lst_of_prog_indices) != 0:
                        trainloader_iter = create_dataloader_during_prog(
                            trainset=self.trainset,
                            lst_of_prog_indices=lst_of_prog_indices,
                            shuffle_ind=cfg.shuffle_ind,
                        ...
                        trainloader_iter = create_dataloader_during_prog(
                            ...
                            shuffle_ind=cfg.shuffle_ind,
                            temp_batch_size=temp_batch_size,
                            mixed_init_training=cfg.mixed_init_training,
                            num_init_conditions=cfg.num_init_conditions,
                        )
            ...
            if cfg.data_type == "colmap":
                ...
                if step % 100 == 0:  # print before u remove stuff
                    ...
                    debug_raster_params["viewmats"] = (
                        torch.from_numpy(random_chosen_camera).to("cuda").to(torch.float32)[None]
                    )  # (1,4,4)
                    debug_raster_params["Ks"] = debug_raster_params["Ks"][0, None]  # (1, 3,3)
                    debug_raster_params["backgrounds"] = debug_raster_params["backgrounds"][
                        0, None
                    ]  # (1,3), single camera
                    viz_renders, _ = self.gaussians.rasterize_with_dynamic_params_batched(
                        pred_param, debug_raster_params, activate_params=True
                    )  # (T,1,H,W,3)
                    ...
                    frames = [img1, img2]
                    imageio.mimsave(f"{path}/train_{first_camera_index_t0}_it{step}.mp4", frames, fps=2, quality=8)
            ...
            if cfg.wandb_every > 0 and step % cfg.wandb_every == 0:
                ...
                n_gauss = len(self.gaussians.splats["means"])
                self.wandb_run.log(
                    {"generate_traj/loss": loss.item(), "generate_traj/num_gs": n_gauss, "generate_traj/mem": mem},
                    step=step,
                )
    ...
    def train_dynamic_blender_masked(
        self,
        init_step=0,
        timestep_counter=None,
        timestep_renders=None,
        ...
        if cfg.mixed_init_training:
            if full_trajectory is None:
                ...
                assert (
                    not cfg.use_progressive_training and not cfg.use_progressive_training_two
                ), "we start with existing trajectory so cannot use progressive training"
        ...
        for step in pbar:
            ...
            if cfg.wandb_every > 0 and step % cfg.wandb_every == 0:
                mem = torch.cuda.max_memory_allocated() / 1024**3
                n_gauss = len(self.gaussians.splats["means"])
                self.wandb_run.log({"train_dyn/loss": loss.item(), "train_dyn/num_gs": n_gauss, "train_dyn/mem": mem})
    ...
    @torch.no_grad()
    def visualize_renderings_captured(
        self,
        fixed_initial_params,
        debug_raster_params,
        bounding_box_mask,
        ...
        for timestep in tqdm.tqdm(range(num_timesteps), desc="viz all timesteps"):
            ...
            if timestep == 0:  # evaluate against static reconstruction's psnr
                cam_image_t0 = torch.from_numpy(self.testset.timestep_images[0][first_test_camera_index]).to(
                    selected_gaussians
                )
                cam_pose_t0 = torch.from_numpy(self.testset.timestep_poses[0][first_test_camera_index]).to(
                    selected_gaussians
                )
                ...
                colors, _ = self.gaussians.rasterize_with_dynamic_params_batched(
                    fixed_initial_params[None], debug_raster_params, activate_params=True
                )
                ...
                first_cam_image_test = torch.from_numpy(
                    self.testset.timestep_images[timestep][first_test_camera_index]
                ).to(torch.float32)
                cont_t = timestep / (num_timesteps - 1)
                cont_t *= self.testset.time_normalize_factor
                inp_t = torch.tensor([0.0, cont_t], dtype=torch.float32, device="cuda")
                pred_param_selected = self.dynamical_model(selected_gaussians, inp_t)  # (T, N_gaussians, feat_dim)
                T = pred_param_selected.shape[0]
                pred_param = fixed_initial_params.unsqueeze(0).repeat(T, 1, 1)
                pred_param[:, bounding_box_mask] = pred_param_selected
        ...
        timestep_data = {"timestep_psnr": timestep_psnr_list, "scene": scene, "num_timesteps": num_timesteps}
        ...
        imageio.mimwrite(
            f"{dynamic_test_path}/{split}_cam0_it{step}.mp4", canvas, fps=canvas.shape[0] / self.cfg.video_duration
        )

        return psnr_ours_dict

    @torch.no_grad()
    def visualize_fixed_pc_traj_captured(
        self, fixed_trajectory, debug_raster_params, step, cfg, path="mixed_init_training"
    ):
        """
        ...
        colors, _ = self.gaussians.rasterize_with_dynamic_params_batched(
            fixed_trajectory_torch, debug_raster_params, activate_params=True
        )  # (T,1,H,W,3)
    ...
    def visualize_fixed_pc_traj(self, fixed_trajectory, debug_raster_params, step, cfg, path="mixed_init_training"):
        ...
        debug_raster_params["backgrounds"] = debug_raster_params["backgrounds"][0, None]  # pick first camera
        colors, _ = self.gaussians.rasterize_with_dynamic_params_batched(
            fixed_trajectory, debug_raster_params, activate_params=True
        )  # (T,1,H,W,3)
        colors = colors.squeeze()
        first_img_ours = torch.clamp(colors, 0, 1).cpu()  # only visualize stuff from first camera.
    ...
    def visualize_renderings_masked(
        ...
        scene = cfg.data_dir.split("/")[-1]
        num_timesteps = (
            self.testset.num_timesteps()
        )  # the number of timesteps is always determined by how many timesteps there is in the testset
