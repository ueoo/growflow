...
from helpers.gsplat_utils import (
    ...
    get_raster_params_captured,
    pers_proj_means,
    world_to_cam_means,
)
from helpers.pc_viz_utils import (
    animate_point_clouds,
    animate_point_clouds_2d,
    animate_point_clouds_captured,
    animate_point_clouds_lst,
    c2w_to_matplotlib_view_colmap,
    c2w_to_matplotlib_view_v2,
    calculate_global_depth_range,
    select_points_in_prism,
...
class Evaluator(BaseEngine):
    def __init__(
        self, cfg, parser, gaussians, dynamical_model, model_list, model_index, trainset, testset, paths, device
    ):
    ...
    def static_eval(self, step: int, time_index, stage: str = "val"):
        ...
        if cfg.data_type == "blender":
            ...
            self.single_timetestset = SingleTimeDataset(
                self.testset, time_index
        ...
        testloader = torch.utils.data.DataLoader(self.single_timetestset, batch_size=1, shuffle=False, num_workers=0)
        elapsed_time = 0
    ...
    def static_render_traj(self, step: int, time_index):
        ...
        if self.parser is not None:
            ...
            camtoworlds_all = np.stack([elem.cpu().numpy() for elem in self.trainset.unique_cameras_lst], axis=0)
        ...
        if cfg.data_type == "colmap":
            if cfg.render_traj_path == "interp":
                ...
                camtoworlds_all = generate_ellipse_path_z(camtoworlds_all, height=traj_height)  # [N, 3, 4]
    ...
    def dynamic_eval_captured(self, fixed_init_params: torch.Tensor, step: int, raster_params, bounding_box_mask):
        ...
        raster_params_test = get_raster_params_captured(
            ...
            if timestep == 0:  # evaluate against static reconstruction's psnr
                for camera_idx in all_test_cam_indices:
                    ...
                    colors, _ = self.gaussians.rasterize_with_dynamic_params_batched(
                        fixed_init_params[None], raster_params_test, activate_params=True
                    )
                ...
                for camera_idx in all_test_cam_indices:
                    ...
                    first_cam_image_test = torch.from_numpy(self.testset.timestep_images[timestep][camera_idx]).to(
                        torch.float32
                    )
                    ...
                    colors, _ = self.gaussians.rasterize_with_dynamic_params_batched(
                        pred_param, raster_params_test, activate_params=True
                    )
        ...
        imageio.mimwrite(f"{path_test}/test_cam0_it{step}.mp4", canvas, fps=canvas.shape[0] / self.cfg.video_duration)

        # 2. Visualize renderings on a train view
        raster_params_train = get_raster_params_captured(
            cfg, self.gaussians.splats, self.testset, self.gaussians.deformed_params_dict
        )  # NOTE: here we still use testset because we just need to append the intrinsics
        ...
        for train_cam_index in all_train_cam_indices[::skip_every]:
            ...
            for timestep in range(num_timesteps):
                ...
                if timestep == 0:  # evaluate against static reconstruction's psnr
                    ...
                    colors, _ = self.gaussians.rasterize_with_dynamic_params_batched(
                        fixed_init_params[None], raster_params_train, activate_params=True
                    )
                    eval_pixels = torch.clamp(cam_image_t0, 0.0, 1.0)
                    ours_t = torch.clamp(colors.squeeze(), 0.0, 1.0)
                    ...
                    first_cam_image_train = torch.from_numpy(
                        self.trainset.timestep_images[timestep][train_cam_index]
                    ).to(torch.float32)
                    cont_t = timestep / (num_timesteps - 1)
                    ...
                    colors, _ = self.gaussians.rasterize_with_dynamic_params_batched(
                        pred_param, raster_params_train, activate_params=True
                    )
                    ours_t = torch.clamp(colors[0, 1], 0, 1).cpu()  # first cam
    ...
    def dynamic_eval(self, fixed_init_params: torch.Tensor, step: int):
        ...
        # out_img: (N, T, H, W, 3)
        out_img, alphas = self.gaussians.rasterize_with_dynamic_params_batched(
        ...
        for i in range(num_cameras):
            ...
            imageio.mimwrite(
                f"{dynamic_test_path}/cam_{i}_val_step{step}.mp4", canvas, fps=out_img.shape[1] / cfg.video_duration
            )
        ...
        # Compute chamfer distance only when mesh assets are available.
        chamfer_distance_lst = []
        avg_chamfer_dist = float("nan")
        gt_tracks_path = os.path.join(cfg.data_dir, "meshes", f"relevant_{scene}_meshes", "trajectory_frames.npz")
        unique_mesh_indices_path = os.path.join(cfg.data_dir, "unique_mesh_indices.npy")
        has_mesh_assets = os.path.exists(gt_tracks_path) and os.path.exists(unique_mesh_indices_path)
        if has_mesh_assets:
            ...
            mesh_indices = np.load(unique_mesh_indices_path).astype(np.uint)
            ...
            for t, mesh_index in enumerate(mesh_indices):
                ...
                gt_mesh_path = os.path.join(
                    cfg.data_dir, "meshes", f"relevant_{scene}_meshes", f"mesh_{converted_integer}.ply"
                )
                chamfer_distance, unscaled_gt_mesh_vert = compute_chamfer_between_point_and_mesh(
                    visible_points, gt_mesh_path, t, cfg.num_vertices_sampled, use_mesh_vertices=cfg.use_mesh_vertices
                )
            ...
            print("mesh assets not found, skipping chamfer/mesh-based evaluation metrics")
    ...
    @torch.no_grad()
    def full_dynamic_eval(
        self,
        fixed_init_params: torch.Tensor,
        step: int,
        num_timesteps=None,
        animate_pc: bool = False,
        offset: int = 0,
        skip_init=False,
    ):
        """
        ...
        if not cfg.skip_test:
            ...
            c2ws, gt_images, inp_t, _, _ = self.testset.__getitems__(
                int_t
            )  # c2ws: [N, 4, 4], gt_images: [N, T, H, W, 3]
            gt_images = gt_images.to(device)
            raster_params["viewmats"] = c2ws.to(device)  # when we rasterize we need to invert this!
            ...
            if cfg.render_foreground:
                ...
                for t, integer_t in zip(inp_t[1:], int_t[1:]):
                    selected_gaussians = fixed_init_params[bounding_box_mask]
                    t_with_zero = torch.tensor([0.0, t.item()], dtype=torch.float32, device="cuda")
                    pred_param_t0_t1 = self.dynamical_model(
                        selected_gaussians, t_with_zero
                ...
                for t, integer_t in zip(inp_t[1:], int_t[1:]):
                    t_with_zero = torch.tensor([0.0, t.item()], dtype=torch.float32, device="cuda")
                    pred_param_t0_t1 = self.dynamical_model(
                        fixed_init_params, t_with_zero
                    )  # (T, N_gaussians, feat_dim)
                    pred_param_t1 = pred_param_t0_t1[1]
                    pred_param[integer_t] = pred_param_t1

            out_img, alphas, lst_of_metas = self.gaussians.rasterize_with_dynamic_params_batched(
                pred_param, raster_params, activate_params=True, return_meta=True
            )
            eval_pixels = gt_images
            colors = torch.clamp(out_img, 0.0, 1.0)
            image_colors = colors
            ...
            for i, cam_id in tqdm(
                ...
                if enable_render_tracks:
                    ...
                    for j in range(len(pred_images) if multi_images else 1):
                        ...
                        if all_trajs is None:
                            ...
                            all_trajs = np.concatenate(
                                (all_trajs, current_means3d[gt_idxs].unsqueeze(0).cpu().numpy()), axis=0
                            )
                        ...
                        if show_flow:
                            ...
                            current_mask = (
                                (current_projections_np[:, 0] >= 0)
                                & (current_projections_np[:, 0] < current_rendering.shape[1])
                                & (current_projections_np[:, 1] >= 0)
                                & (current_projections_np[:, 1] < current_rendering.shape[0])
                            )
                            ...
                            if all_trajs.shape[0] > 1:
                                traj_img = np.ascontiguousarray(
                                    np.zeros(
                                        (current_rendering.shape[0], current_rendering.shape[1], 3), dtype=np.uint8
                                    )
                                )
                                ...
                                for t_idx in range(all_trajs.shape[0] - 1):  # loop over each time
                                    ...
                                    prev_projections = pers_proj_means(
                                        prev_projections_cam, raster_params["Ks"][0][None], 400, 400
                                    )
                                    ...
                                    curr_projections = pers_proj_means(
                                        curr_projections_cam, raster_params["Ks"][0][None], 400, 400
                                    )
                                    ...
                                    prev_mask = (
                                        (prev_projections[:, 0] >= 0)
                                        & (prev_projections[:, 0] < current_rendering.shape[1])
                                        & (prev_projections[:, 1] >= 0)
                                        & (prev_projections[:, 1] < current_rendering.shape[0])
                                    )
                                    curr_mask = (
                                        (curr_projections[:, 0] >= 0)
                                        & (curr_projections[:, 0] < current_rendering.shape[1])
                                        & (curr_projections[:, 1] >= 0)
                                        & (curr_projections[:, 1] < current_rendering.shape[0])
                                    )
                                    ...
                                    if curr_time <= view_time and prev_time <= view_time:
                                        for idx in range(0, curr_projections.shape[0], flow_skip):
                                            color_idx = (idx // flow_skip) % len(colors)
                                            if (
                                                prev_mask[idx] and curr_mask[idx]
                                            ):  # whether or not we plot current gaussian
                                                traj_img = cv2.line(
                                                    traj_img,
                                                    (
                                                        int(prev_projections[idx, 0]),
                                                        int(prev_projections[idx, 1]),
                                                    ),  # start
                                                    (
                                                        int(curr_projections[idx, 0]),
                                                        int(curr_projections[idx, 1]),
                                                    ),  # end
                                                    tuple(int(c * fade_factor) for c in colors[color_idx]),
                                                    arrow_thickness,
                                                )  # overlay line on black image
                        ...
                        if multi_images:
                            j_plus_offset = j + offset
                            imageio.imwrite(f"{track_cam_test_path}/{j_plus_offset:05d}.png", current_rendering)
                        else:
                            j_plus_offset = offset
                            imageio.imwrite(f"{track_cam_test_path}/{j_plus_offset:05d}.png", current_rendering)
                    ...
                    imageio.mimwrite(
                        f"{track_cam_test_path}/full_track.mp4",
                        per_camera_tracks_imgs,
        ...
        if enable_animate_pc:
            ...
            for i in range(num_test_cameras):
                ...
                current_means_cam = world_to_cam_means(
                    self.gaussians.splats.means[gt_idxs_viz_pc], first_w2c[None].cuda()
                )
    ...
    @torch.no_grad()
    def full_dynamic_eval_captured(
        self,
        fixed_init_params: torch.Tensor,
        step: int,
        num_timesteps=None,
        ...
        raster_params = get_raster_params_captured(
            cfg, self.gaussians.splats, self.testset, self.gaussians.deformed_params_dict
        )
        ...
        if not cfg.skip_test:
            ...
            out_img = torch.zeros(
                num_test_timesteps, number_of_cameras, raster_params["height"], raster_params["width"], 3
            )
            ...
            for t in range(num_test_timesteps):
                ...
                if inp_t == 0:  # dont query neural ode
                    ...
                    t_with_zero = torch.tensor([0.0, inp_t], dtype=torch.float32, device="cuda")
                    selected_gaussians = fixed_init_params[bounding_box_mask]
                    pred_param_t0_t1 = self.dynamical_model(
                        selected_gaussians, t_with_zero
                    )  # (T, N_gaussians, feat_dim)
                ...
                renders, alphas = self.gaussians.rasterize_with_dynamic_params_batched(
                    pred_param_t1[None], raster_params, activate_params=True, return_meta=False
                )
            ...
            for i, cam_id in tqdm(
                ...
                if cfg.render_tracks:
                    ...
                    for j in range(len(pred_images) if multi_images else 1):
                        ...
                        # Project current 3D positions to 2D
                        current_means_cam = world_to_cam_means(current_means3d[gt_idxs], current_viewmat[None])
                        means_2d = pers_proj_means(
                            current_means_cam, raster_params["Ks"][0][None], width=width, height=height
                        ...
                        if show_flow:
                            ...
                            if all_trajs.shape[0] > 1:
                                ...
                                for t_idx in range(all_trajs.shape[0] - 1):  # loop over each time
                                    ...
                                    prev_projections_cam = world_to_cam_means(prev_gaussians, current_viewmat[None])
                                    prev_projections = pers_proj_means(
                                        prev_projections_cam, raster_params["Ks"][0][None], width=width, height=height
                                    )
                                    prev_projections = prev_projections.squeeze()
                                    prev_time = all_times[t_idx]
                                    ...
                                    curr_projections_cam = world_to_cam_means(curr_gaussians, current_viewmat[None])
                                    curr_projections = pers_proj_means(
                                        curr_projections_cam, raster_params["Ks"][0][None], width=width, height=height
                                    )
                                    curr_projections = curr_projections.squeeze()
                                    curr_time = all_times[t_idx + 1]
            ...
            if cfg.animate_pc:
                ...
                for i in range(num_test_cameras):
                    ...
                    first_w2c = torch.linalg.inv(first_c2w)
                    current_means_cam = world_to_cam_means(
                        self.gaussians.splats.means[bounding_box_mask], first_w2c[None].cuda()
                    )
                    means_2d = pers_proj_means(
                        current_means_cam,
                        raster_params["Ks"][0][None].cuda(),
                    ...
                    pixel_coords[:, 0] = torch.clamp(
                        pixel_coords[:, 0], 0, raster_params["width"] - 1
                    )  # x coordinates
                    pixel_coords[:, 1] = torch.clamp(
                        pixel_coords[:, 1], 0, raster_params["height"] - 1
                    )  # y coordinates
                    closest_pixels = view_image[pixel_coords[:, 1], pixel_coords[:, 0]]  # Shape: (N, 3)
                    ...
                    overlay_image[pixel_coords[:, 1], pixel_coords[:, 0]] = torch.tensor(
                        [1.0, 0.0, 0.0], device=view_image.device
                    )  # Red points
                    ...
                    current_means_cam_all = world_to_cam_means(
                        visible_points, first_w2c[None, None].expand(T, -1, -1, -1).cuda()
                    ).squeeze()  # (T, N, 3)
                    means_2d_all = pers_proj_means(
                        current_means_cam_all, intrns, width=raster_params["width"], height=raster_params["height"]
                    ).squeeze()
                    animate_point_clouds_2d(
                        means_2d_all,
                        output_file=f"{test_eval_path}/point_cloud_gs_color_animation_r_{i}.mp4",
        ...
        if cfg.render_spacetime_viz:
            ...
            for i, img in tqdm(enumerate(out_img), total=(out_img.shape[0])):
                ...
                pred_image = (pred_images * 255).cpu().numpy().astype(np.uint8)
                imageio.imwrite(f"{space_time_viz_path}/{i:05d}.png", pred_image)
        ...
        if cfg.render_demo_viz:
            ...
            fixed_viewpoint = (
                torch.from_numpy(self.parser.camtoworlds[-1:]).repeat(extra_cameras, 1, 1).to(torch.float32)
            )
    ...
    def full_render_gt(self):
            ...
            if cfg.data_type == "blender":
                ...
                for i, cam_id in tqdm(
                    ...
                    for j, frame in enumerate(gt_image):
                        ...
                        imageio.imwrite(f"{cam_test_path}/{j:05d}.png", gt_image)  # ranges from [0,17]
            else:
                for t in int_t:
                    ...
                    for view in range(num_views):
                        ...
                        imageio.imwrite(f"{cam_test_path}/{t:05d}.png", image)
        ...
        if cfg.animate_pc:
            ...
            if "clematis" in scene:
                ...
                center_position = [-1.24790855e-02, 6.82123005e-04, 1.60255575e00]
            elif "plant_2" in scene:
                center_position = [2.4079531e-04, -6.9841929e-03, 1.6393759e00]
            elif "plant_3" in scene:
                center_position = [-1.5169904e-03, 7.5232387e-03, 1.6430800e00]
            ...
            pose_dict = {
                "r_0": [15.732388496398926, -86.39990997314453],
