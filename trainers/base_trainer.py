...
class BaseEngine:
    ...
    def __init__(self, cfg, gaussians, dynamical_model, model_list, model_index, trainset, testset, paths, device):
        ...
        if isinstance(self.cfg.strategy, DefaultStrategy):
            self.strategy_state = self.cfg.strategy.initialize_state(scene_scale=self.gaussians.scene_scale)
        ...
        self.fixed_bkgd = torch.tensor(cfg.bkgd_color, device=self.device)[None, :]
        assert (self.fixed_bkgd <= 1).all() and (
            self.fixed_bkgd >= 0
        ).all(), "bkgd color needs to be normalized bw [0,1]"

        # Metrics, for psnr, we use our own implementation as it supports mask psnr
        self.ssim = StructuralSimilarityIndexMeasure(data_range=1.0).to(self.device)
        self.psnr = psnr
        self.lpips = LearnedPerceptualImagePatchSimilarity(net_type="vgg", normalize=True).to(
            self.device
