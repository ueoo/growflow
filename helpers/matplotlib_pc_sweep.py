...
def sweep_camera_conventions(
    ...
    print(
        f"Total combinations to test: {len(forward_directions) * len(forward_negations) * len(azimuth_axes) * len(elevation_axes) * len(azimuth_offsets) * len(elevation_modifications)}"
    )
    ...
    with open(log_file, "w") as log:
        ...
        for idx, (name, fwd_col, negate, az_offset, elev_neg) in enumerate(common_conventions):
            try:
                ...
                log_entry += (
                    f"  Forward col: {fwd_col}, Negate: {negate}, Az offset: {az_offset}°, Elev negate: {elev_neg}\n"
                )
                log_entry += f"  Elevation: {elevation:.2f}°, Azimuth: {azimuth:.2f}°\n"
                log_entry += f"  Output: {Path(output_file).name}\n\n"
                log.write(log_entry)
