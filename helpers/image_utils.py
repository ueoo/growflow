...
def video_dir_to_tensor(video_path: str, shape=(256, 256), return_trajectory=False, is_reverse=False):
    ...
    if len(all_npy_files) > 0:
        ...
        images[images <= 0.05] = (
            0  # even though the images are generated from coarse stage, it's good to clean so we get good psnr
        )
        ...
        h, w = shape[0], shape[1]
        transform = transforms.Compose([transforms.Resize((h, w)), transforms.ToTensor()])
        img_files = sorted([f for f in os.listdir(video_path) if f.endswith(".png")])
        img_frames = []
        # Process images
...
def create_trajectory(gs, traj_func, num_timesteps=40):
    ...
    means = (0.5 * torch.tanh(gs.gaussian_params[:, 3:5].detach().cpu()) + 0.5) * torch.tensor(
        gs.img_size[:2][::-1]
    ).detach().cpu()
...
def zigzag_trajectory(t, initial_state, end_state, scale_factor=1.0):
    ...
    assert torch.allclose(
        trajectory[0], initial_state, rtol=1.0e-5, atol=1.0e-5
    ), "trajectory at time 0 doesn't equal initial state"
...
def star_of_david_trajectory(t, initial_state, end_state, scale_factor=1.0):
    ...
    pattern = pattern_base[indices] * (1 - fractions.unsqueeze(-1)) + pattern_base[
        (indices + 1) % 6
    ...
    assert torch.allclose(trajectory[0], initial_state, rtol=1.0e-5, atol=1.0e-5)
