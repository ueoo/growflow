...
def curriculum_training_strategy(total_iterations, num_timesteps, base=1, min_iterations=500):
    ...
    if total_iterations < required_min_iterations:
        raise ValueError(
            f"Total iterations ({total_iterations}) must be at least {required_min_iterations} "
...
def generate_linear_sequence(target_sum, n, start=None):
    ...
    while target_sum - n * a < (
        n * (n - 1) / 2
    ):  # we want the ratio to be at least the denominator, so we have at least increment of 1
        a -= 1
    ...
    assert (
        sum(integer_list) == target_sum
    ), "something is wrong, the sum of our integer list is not equal to the target"
    return integer_list
