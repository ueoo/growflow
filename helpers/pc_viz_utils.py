    ...
    if use_z_coloring:
        ...
        if depth_reference_point is None:
            if (min_vals is not None) and (max_vals is not None):
                ...
                raise ValueError(
                    "min_vals and max_vals must be provided for depth coloring if no reference point is given"
                )
...
def visualize_point_cloud_captured(
    ...
    if use_z_coloring:
        ...
        if global_depth_min is not None and global_depth_max is not None:
            scatter = ax.scatter(
                x, y, z, s=point_size, c=depths, cmap=colormap, vmin=global_depth_min, vmax=global_depth_max, alpha=0.8
            )
        else:
...
def generate_rotation_sequence(
    point_cloud_sequence, legend_labels, base_filename="pc_rotation", output_dir="rotation_frames"
):
    """
    ...
    print(
        f"Frames 00001-{current_index-1:05d} show spiral motion from {start_elevation}° to {end_elevation}° elevation"
    )
...
def visualize_point_cloud(
    ...
    if use_z_coloring:
        scatter = ax.scatter(
            x, y, z, s=point_size, c=-z, cmap=colormap, alpha=0.8
        )  # keep the convention that warmer colors at the bottom and colder colors top
    elif use_depth_coloring:
    ...
    plt.savefig(output_file, dpi=300, pad_inches=0, facecolor="none", edgecolor="none")  # Removed bbox_inches="tight"
    print(f"Visualization saved to {output_file}")
...
def visualize_point_cloud_toy(
    ...
    # Save with minimal padding
    plt.savefig(output_file, dpi=300, pad_inches=0, facecolor="none", edgecolor="none")
    print(f"Visualization saved to {output_file}")
...
def visualize_point_trajectory(trajectory2d, num_pts, path, img_shape, iteration=0):
    ...
    for i in range(num_pts):  # start from the top
        ...
        ax.plot(
            pc[:, 0], pc[:, 1], "-", color=colors[i], label=f"Point {i}", alpha=1  # all x,y coords
        )  # Slight transparency

        # Add start point
        ax.plot(pc[0, 0], pc[0, 1], "o", color=colors[i], markersize=10, markeredgecolor="white", markeredgewidth=2)
    ...
    plt.savefig(f"{path}/trajectories_{iteration}.png", dpi=100, bbox_inches="tight", pad_inches=0.2)
    plt.close()
...
def select_points_in_prism(points, center, dimensions, rotation_angles=(0, 0, 0)):
    ...
    # Create mask for points within the box
    mask = np.all((rotated_points >= min_bounds) & (rotated_points <= max_bounds), axis=1)
...
def c2w_to_matplotlib_view_v2(c2w_matrix):
    ...
    elevation = np.degrees(np.arctan2(forward_vec[2], np.sqrt(forward_vec[0] ** 2 + forward_vec[1] ** 2)))
...
if __name__ == "__main__":
    point_clouds = torch.load(
        "/scratch/ondemand28/weihanluo/2dgsplat/results/rose_2025_03_13-12_35_50/point_cloud_trajectory.pt",
        weights_only=True,
    )
