...
def create_splats_with_optimizers(
    ...
    if init_type == "sfm":
        ...
        mesh = trimesh.load_mesh(
            first_mesh_path, process=True
        )  # NOTE: must set process=True, otherwise duplicate gaussians
...
def prepare_times(
    ...
    check_fixed_initial_params(
        gaussians, fixed_initial_params, deformed_params_dict, deformed_params_list
...
def get_raster_params_blender(cfg, gaussians, dataset, deformed_params_dict):
    ...
    raster_params["sh_degree"] = (
        cfg.sh_degree
    )  # this is fine since get_raster_params is called during dyn train, which all deg have been act
    ...
    raster_params["backgrounds"] = (
        torch.tensor(cfg.bkgd_color, device="cuda")[None].expand(num_cameras, -1).to(torch.float32)
    )
...
def get_raster_params_captured(cfg, gaussians, dataset, deformed_params_dict):
    ...
    if dataset.split == "train":  # need to use camera_filter here to select the first training camera
        num_cameras_in_batch = (
            len(dataset.camera_filter[0]) if dataset.cam_batch_size == -1 else dataset.cam_batch_size
        )
    else:
    ...
    raster_params["Ks"] = torch.from_numpy(dataset.timestep_intrinsics[first_timestep][1][None]).expand(
        num_cameras_in_batch, -1, -1
    )  # (num_cam, 3, 3)
    ...
    raster_params["absgrad"] = cfg.strategy.absgrad if isinstance(cfg.strategy, DefaultStrategy) else False
    raster_params["backgrounds"] = (
        torch.tensor(cfg.bkgd_color, device="cuda")[None].expand(num_cameras_in_batch, -1).to(torch.float32)
    )
    # NOTE: for c2w, we append them during training
...
def create_dataloader_during_prog(
    trainset, lst_of_prog_indices, shuffle_ind, temp_batch_size, mixed_init_training=False, num_init_conditions=4
):
    ...
    if mixed_init_training:
        ...
        prog_sampler = NeuralODEDataSampler_MixedInit(
            trainset, lst_of_prog_indices, seq_length=temp_batch_size, num_initial_conditions=num_init_conditions
        ...
        prog_sampler = InfiniteNeuralODEDataSampler(trainset, lst_of_prog_indices, shuffle=shuffle_ind)
...
def world_to_cam_means(means, viewmats):
...
def pers_proj_means(
    ...
    # Project to 2D
    means2d = torch.einsum("...ij,...nj->...ni", Ks[..., :2, :3], torch.stack([tx, ty, tz], dim=-1))  # [..., C, N, 2]
    means2d = means2d / tz[..., None]  # [..., C, N, 2]
